...
\ifxetex
  % If using XeTeX, use system fonts that are widely available
  \setmainfont{Arial}[
    Ligatures=TeX,
    BoldFont=Arial Bold,
...
\else
  \ifluatex
    % If using LuaTeX, use system fonts that are widely available
    \setmainfont{Arial}[
      Ligatures=TeX,
      BoldFont=Arial Bold,
      ItalicFont=Arial Italic,
    ...
    \setsansfont{Arial}[
      ...
      BoldItalicFont=Arial Bold Italic]
  \else
    % If using PDFLaTeX, use standard LaTeX fonts
    \usepackage{lmodern}
  \fi
\fi
